...
def calculate_smart_energy(
    ...
    """智能照明能耗：读取真实仿真/运行遥测（brightness、power、timestamp），
    按 power 字段对每个采样周期积分得到全天能耗（kWh）。

    是 ``calculate_total_energy`` 的任务书命名版本，用于与
    ``calculate_traditional_energy`` / ``calculate_saving_rate`` 配套调用；
    两者计算逻辑完全一致，仅作为对外统一的函数名。telemetry 记录需至少包含
    power 字段用于积分；brightness / timestamp 字段用于校验数据确实来自
    仿真遥测（而非手工估算值），存在但为 None 时不参与计算。
    """
    telemetry_list = list(telemetry_list)
    for telemetry in telemetry_list:
        missing = [field for field in ("brightness", "power", "timestamp") if field not in telemetry]
        if missing:
            raise KeyError(f"Telemetry 缺少字段: {', '.join(missing)}")
    return calculate_total_energy(telemetry_list, step_minutes)
...
def calculate_traditional_energy(devices: Mapping[str, Mapping] = DEVICES) -> float:
    """传统照明能耗：模拟 07:00~22:00 所有灯具 100% 亮度全开的日能耗（kWh）。

    是 ``calculate_traditional_daily_energy`` 的任务书命名版本，参数、公式和
    返回值完全一致（时长和亮度取自 ``config.ENERGY``），仅作为对外统一的函数名。
    """
    return calculate_traditional_daily_energy(devices)
...
__all__ = [
    ...
    "calculate_smart_energy",
    "calculate_total_energy",
    "calculate_traditional_daily_energy",
    "calculate_traditional_energy",
