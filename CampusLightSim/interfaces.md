...
> 2026-09-11 全量接入更新：**1,690 个控制节点全部实例化**，对应8,363盏灯具，每间教室一个节点（主楼100间）。`planning.py` 定义38项清单，`deployment.py` 展开独立设备、控制区和传感器，`config.DEVICES/ZONES` 均为1,690项。原CL-N01～CL-N10保留为各类第一条真实回路，不另加10个代表设备。每日5分钟步长共486,720条采样，能耗按全部回路灯数×单灯功率重新计算。详细依据见 [照明数量估算清单](docs/lighting_quantity_plan.md)。以下V1示例保留作接口参考，其中旧规模与功率数值以当前配置为准。
